...
class RuntimeStateNormalizationTests(unittest.TestCase):
    ...
    def test_unreal_message_false_updates_flag_to_false(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            manager = self._build_manager(Path(temp_dir) / "current_response.wav")
            try:
                ...
                self.assertFalse(manager.is_ai_realtime_processing_enabled())
            finally:
                manager.shutdown()
    ...
    def test_unreal_message_string_false_updates_flag_to_false(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            manager = self._build_manager(Path(temp_dir) / "current_response.wav")
            try:
                handled = manager.handle_unreal_websocket_message(
    ...
    def test_tts_realtime_true_uses_realtime_audio_mode(self) -> None:
        ...
        self.assertEqual(stream_call["websocket_audio_chunk_ms"], 20)
